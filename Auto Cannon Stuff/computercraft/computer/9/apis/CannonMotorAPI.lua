...
os.loadAPI("libs/CannonMotorLib.lua")
----Config----
--Yaw
local yawMotor = {
...
function InputChannel(targetAngle,motor)
    --if(CannonMotorLib.AngleInRange(MathLib.NormaliseAngle180(targetAngle),motor.min,motor.max)) then return end
    ...
    motor.curDeg = MathLib.NormaliseAngle360(motor.curDeg + CannonMotorLib.TryRotateMotor(rotationA,rpm,motor))
end


function GetAngles()
    return yawMotor.curDeg, pitchMotor.curDeg
end
